...
%% behavioral index - seq, seq_id
if ~isempty(seq)
    temp_ind = [];
    cseq     = [0 cumsum(seq)];
    for i=2:length(seq_id)+1
        if seq_id(i-1)==1
            temp_ind = [temp_ind cseq(i-1)+1:cseq(i)];
        end
    end
    Xs = Xs(:,temp_ind,:);
    ValidId=temp_ind;
else
    ValidId=1:length(XPos);
end
...
save(Name,'eParam','sParam','dValid','data_type','ModelSetting','training_ind','SampleX','ValidId');
