function Features=ay_neural_encoder_training_shuffledtrials(file_name,ModelSetting,training_ind,NPerm)
...
for nn=1:NPerm
    ...
    id=randperm(size(Y,1),length(training_ind));
    disp(['N=',num2str(nn)])
    for fn = 1:no_feature
    ...
    Features=[Features,length(find(dValid(:,1)==1))];
end
